...
- **Signal Lab (2026-07-21)** — `/lab`, the track record of Catalyst's OWN signals. `lib/signals/`: `detect.ts` registers a `signal_events` row the moment a signal fires (every cron + refresher tick, no LLM), `outcomes.ts` measures it later against prices. **The registry is prospective and never revised** — that's what keeps it free of the lookahead bias that contaminates LLM backtests, so don't add a path that rewrites or re-scores past events.
  ...
  - **Prices**: `lib/signals/prices.ts` — Yahoo (`getDailyAdjCloses`) → FMP `historical-price-eod/dividend-adjusted`. Yahoo rate-limits **by IP** with no warning (429 to every chart request from the user's Mac, 2026-07-21) while GH runners are fine, which is why the outcomes job lives in the cron. The FMP fallback is **off by default** (`LAB_FMP_MAX_CALLS=0`): its budget is per-process and the cron runs ~144×/day against a 250/day quota shared with fundamentals. Enable it only for manual local runs. **Stooq is not an option** — it now serves a JS proof-of-work challenge (fragile scraping).
