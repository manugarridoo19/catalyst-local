...
- **LLM stack (2026-07-16)**: OpenRouter primary → **Gemini** (Google AI Studio, `lib/providers/gemini.ts`) → Groq last resort (`SCORER_PRIMARY` env overrides the head). The Gemini pool is 3 AI Studio keys rotated **round-robin per request** (its binding limit is RPM, unlike OpenRouter's daily cap — round-robin makes N keys ≈ N× RPM); model `gemini-3.1-flash-lite` (successor of 2.5-flash-lite, which is closed to new accounts), fallback `gemini-2.0-flash-lite`. 429s are classified: daily quota → cool key until Pacific midnight (~07:05Z), RPM burst → cool `retryDelay`+2s. Keys: `GEMINI_API_KEYS` env (GH Secrets, Worker secret) or `~/.catalyst-gemini-keys` (mode 600). Every Gemini request sends `thinkingConfig:{thinkingBudget:0}` — same reasoning-burn issue as Nemotron. User-facing prose goes through `lib/ai/prose-chain.ts` (openrouter task="brief" → gemini → groq 70b → 8b). OpenRouter model chains are **per task** in `lib/providers/openrouter.ts` (`chatCompletion({task})`):
